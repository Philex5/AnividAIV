...
import { buildOptimizationPrompt } from "@/lib/prompt-optimizer";
  ...
  try {
    ...
    try {
      const { text } = await generateTextWithFallback({
        ...
        temperature: 0.7,
        provider: "xiaojing",
        scenario: "anime_generation_optimize_prompt",
      });
      ...
      // 如果LLM调用失败，回退到简单优化
      optimizedPrompt = `${validatedData.prompt}, detailed, high quality, digital art`;
      improvements = [
        "Added basic quality enhancement terms",
