...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const result of results) {
      ...
      if (!result.image_url) {
        avatarMap[result.character_uuid] = null;
        continue;
      }
      ...
      avatarMap[result.character_uuid] = toImageUrl(selectedUrl);
    }

    // 确保所有请求的角色都有对应的条目
    for (const uuid of characterUuids) {
      if (!(uuid in avatarMap)) {
        avatarMap[uuid] = null;
      }
    }
    ...
    console.error("Failed to get character avatars in batch:", error);
    return NextResponse.json(
