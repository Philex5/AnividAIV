...
import { getOCMakerPage } from "@/services/page";
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: `${process.env.NEXT_PUBLIC_WEB_URL}/oc-maker`,
        ja: `${process.env.NEXT_PUBLIC_WEB_URL}/ja/oc-maker`,
      },
    },
    openGraph: {
      title,
...
export default async function OCMakerPage({
  params,
  searchParams,
}: {
  params: Promise<{ locale: string }>;
  ...
  const structuredTitle =
    pageData.metadata?.title || "OC Maker on AnividAI | Anime Character Creator";
  const structuredDescription =
    pageData.metadata?.description ||
    pageData.Introduce?.description ||
    "Create original characters with AnividAI's OC Maker.";
  ...
  const softwareApplicationJsonLd = {
    ...
    "alternateName":
      pageData.Introduce?.tagline || pageData.metadata?.title || "OC Maker",
