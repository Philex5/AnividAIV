...
export async function updateOrderSubscription(
  ...
  if (isNaN(expired_at.valueOf())) {
    throw new Error("invalid sub_period_end for order subscription update");
  }
...
export async function getPaidOrdersTotal(): Promise<number | undefined> {
  const total = await db().$count(orders, eq(orders.status, OrderStatus.Paid));
...
export async function getOrderCountByDate(
  ...
  const whereCondition = status
    ? and(gte(orders.created_at, new Date(startTime)), eq(orders.status, status))
    : gte(orders.created_at, new Date(startTime));
