/**
 * Flux 2 Pro Adapter
 * Handles flux-2/pro-text-to-image and flux-2/pro-image-to-image model API calls
 */
...
interface Flux2ProCreateResponse {
  code: number;
  message?: string;
  data?: {
    taskId: string;
...
interface Flux2ProQueryResponse {
  code: number;
  message?: string;
  data?: {
    taskId: string;
...
export class Flux2ProAdapter extends BaseAdapter {
  private readonly createEndpoint = "/api/v1/jobs/createTask";
  private readonly queryEndpoint = "/api/v1/jobs/recordInfo";
  private readonly modelIdImageToImage = "flux-2/pro-image-to-image";
  private readonly modelIdTextToImage = "flux-2/pro-text-to-image";
  ...
  async createTask(
    ...
    const response = await this.fetchAPI<Flux2ProCreateResponse>(
    ...
    if (response.code !== 200 || !response.data?.taskId) {
      throw new Error(
        `Failed to create Flux 2 Pro task: ${(response as any)?.message || "Unknown error"}`
      );
    }
  ...
  async queryTask(taskId: string): Promise<TaskQueryResult> {
    ...
    const response = await this.fetchAPI<Flux2ProQueryResponse>(url, {
      method: "GET",
    });

    if (response.code !== 200 || !response.data) {
      throw new Error(
        `Failed to query Flux 2 Pro task: ${(response as any)?.message || "Unknown error"}`
      );
    }
  ...
  protected mapAspectRatio(aspectRatio: string): string {
    ...
    if (!supportedRatios.includes(aspectRatio)) {
      throw new Error(`Unsupported Flux 2 Pro aspect ratio: ${aspectRatio}`);
    }
  ...
  private mapResolution(resolution?: string): string {
    ...
    if (normalized !== "1K" && normalized !== "2K") {
      throw new Error(`Unsupported Flux 2 Pro resolution: ${resolution}`);
    }
